...
class ALA:
    ...
    def adaptive_local_aggregation_aaw(self,
        ...
        Generates the Dataloader for the randomly sampled local training data and
        preserves the lower layers of the update.
        与FedAvg类似直接将全局模型参数完全赋值给本地模型（静态）
        ...
        Returns:
            None.
        """
        print("use global model init local model")
        params_g = list(global_model.parameters())
        ...
        for param_t, param_l, param_g in zip(params_t, params_l,params_g):
            #param_t.data = param_g + (param_g - param_l) * 0
