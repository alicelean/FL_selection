...
def separate_data_origin(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=2):
    '''
    ...
    '''
    print(f"niid is {niid},balance is {balance},least_samples{least_samples}")
...
def separate_data_1(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=2):
    '''
    将数据集按照指定的方式划分为多个客户端，并进行数据的非独立和平衡性处理
    Args:
        data:一个元组，包含图像数据和标签数据。
    ...
    if partition == 'pat':
        ...
        while flag:
            ...
            clients_per_class = int(num_clients*0.2)
            # 初始化每个客户端的样本总量
            total_samples_per_client = [0] * num_clients
            # 初始化每个客户端可分配的类别数量
            remaining_classes_per_client = [clients_per_class] * num_clients
            # 存储每个客户端分配的样本索引
            dataidx_map = {client: [] for client in range(num_clients)}
            ...
            for k in range(num_classes):
                ...
                # 当前类别的可用客户端
                available_clients = [i for i in range(num_clients) if remaining_classes_per_client[i] > 0]

                # 从可用客户端中随机选择最多 max_clients_per_category 个客户端
                selected_clients = np.random.choice(
                    available_clients,
                    size=min(len(available_clients), clients_per_class),
                    replace=False
                )

                # 随机生成每个选中客户端分配的比例
                num_selected_clients = len(selected_clients)
                proportions = np.random.dirichlet(np.ones(num_selected_clients) * 0.5)  # 非均匀比例
                proportions = (proportions * len(idx_k)).astype(int)  # 转为样本数量
                ...
                # 分割样本并分配到客户端
                start_idx = 0
                for client, num_samples in zip(selected_clients, proportions):
                    if num_samples > 0:
                        dataidx_map[client].extend(idx_k[start_idx:start_idx + num_samples].tolist())
                        start_idx += num_samples
                        # 更新客户端的总样本数
                        total_samples_per_client[client] += num_samples
                        # 减少该客户端可以分配的类别数量
                        remaining_classes_per_client[client] -= 1

                # 二次处理逻辑
            while True:
                # 找到样本数不足的客户端
                underfilled_clients = [i for i, count in enumerate(total_samples_per_client) if
                                       count < least_samples]
                if not underfilled_clients:
                    break  # 所有客户端已满足条件

                # 找到样本数超过 2 * min_samples_per_client 的客户端
                overfilled_clients = [i for i, count in enumerate(total_samples_per_client) if
                                      count > 2 * least_samples]
                if not overfilled_clients:
                    raise ValueError("无法满足所有客户端的最小样本要求，因为没有足够的可用样本。")

                # 从过多样本的客户端中分配样本给不足的客户端
                for under_client in underfilled_clients:
                    deficit = least_samples - total_samples_per_client[under_client]
                    for over_client in overfilled_clients:
                        if deficit <= 0:
                            break  # 当前不足的客户端已补充完成

                        # 从过多的客户端中分配样本
                        surplus = total_samples_per_client[over_client] -  least_samples
                        transfer_samples = int(min(deficit, surplus))
                        if transfer_samples > 0:
                            # 转移样本
                            print(f"dataidx_map[over_client] is {dataidx_map[over_client]},transfer_samples is {transfer_samples}")
                            transfer_data = dataidx_map[over_client][:transfer_samples]
                            dataidx_map[over_client] = dataidx_map[over_client][transfer_samples:]  # 剩余样本
                            dataidx_map[under_client].extend(transfer_data)

                            # 更新样本总数
                            total_samples_per_client[over_client] -= transfer_samples
                            total_samples_per_client[under_client] += transfer_samples
                            deficit -= transfer_samples


            print(f"dataidx_map is {dataidx_map}")
            for i in range(num_clients):
                idx_batch[i]=dataidx_map[i]
            flag=False
    ...
    if partition == 'pat':
        ...
        while flag:
            ...
            category_distribution = {
                1: int(num_clients * 0.2),  # 20% 客户端有 1 个类别
                2: int(num_clients * 0.4),  # 40% 客户端有 2 个类别
                4: int(num_clients * 0.1),  # 10% 客户端有 4 个类别
                6: int(num_clients * 0.1),  # 8% 客户端有 6 个类别
                num_classes: int(num_clients * 0.2)  # 2% 客户端有所有类别
            }
            ...
            client_categories = []
            # 根据比例分配类别数给客户端
            for num_categories, count in category_distribution.items():
                client_categories.extend([num_categories] * count)
            # 如果分配的客户端总数不足，补齐到 num_clients
            while len(client_categories) < num_clients:
                client_categories.append(1)
            ...
            total_samples_per_client = [0] * num_clients
            # 初始化每个客户端可分配的类别数量
            remaining_classes_per_client = client_categories
