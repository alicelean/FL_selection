    ...
    def train(self):
        ...
        for i in range(self.global_rounds+1):
            ...
            if i%self.eval_gap == 0:
                ...
                train_loss, test_acc, _=self.evaluate_personalized()
                # return [self.method, group, train_loss, test_acc, test_auc, np.std(accs), np.std(aucs)]
                # --------------------4.记录当前模型的状态，loss,accuracy----------------------
                resc = [self.filedir,self.method, i,train_loss,test_acc,"0_auc","0_std","0_std",self.join_ratio]
