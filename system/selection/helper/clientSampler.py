from selection.helper.client import Client
import math
...
from selection.helper.oort import create_training_selector
        ...
        if not even:
            for key in self.clientOnHosts.keys():
                for client in self.clientOnHosts[key]:
                    ...
                    #print(f"client {uniqueId},size is {self.Clients[uniqueId].size},{self.Clients[self.getUniqueId(hostId, clientId)].size},totalSampleInTraining is  {totalSampleInTraining}")
