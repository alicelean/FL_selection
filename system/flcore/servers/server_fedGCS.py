...
from flcore.clients.client_fedGCS import clientFedGCS
...
import random,copy
from selection.fedGCS.records_collector import process
from selection.fedGCS.autos.train import autos_selection
...
class FedGCS(Server):
    ...
    def collect_train(self,collect_round):
       #新增1————————————————————————————————————————————————————————————————————————————————
        colum_value = []
        select_id = []
       # ————————————————————————————————————————————————————————————————————————————————

        for i in range(collect_round+1):
            i += 1
            self.current_round=i
            if self.cost_time > self.total_time or round == collect_round:
                break
            s_t = time.time()
            #训练之前先计算客户端的状态
            # 新增2————————————————————————————————————————————————————————————————————————————————
          ...
          # for client in self.selected_clients:
            ...
            if i%self.eval_gap == 0:
                ...
                print("Accuracy is :",res[3])
                # 新增3————————————————————————————————————————————————————————————————————————————————
                self.record.append([ids,self.get_pi(res[3])])
        ...
        process(self.record, self.num_clients,self.programpath)
        print("\nBest accuracy.")
    ...
    def get_pi(self,acc):
        ...
        for client in self.selected_clients:
            PL+=client.latency
            PE+=client.Energycost
        if self.L<PL:
            a=math.pow(float(self.L)/PL, 2)
        if self.E<PE:
            b=math.pow(float(self.E)/PE, 2)
        pi=acc*a*b
